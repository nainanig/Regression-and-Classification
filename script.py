...
def regressionObjVal(w, X, y, lambd):
    ...
    #print("temp2_trans",np.shape(temp2_trans))
    temp3=np.dot(temp2_trans,temp2);
    w_trans=np.transpose(w);
    t4=np.dot(w_trans,w);
    t5=lambd*t4;
    #print("t5",np.shape(t5))
    error=temp3+t5;
    #print("error",np.shape(error_grad))

    x_trans=np.transpose(X);
    #print("temp 2 ",np.shape(temp2))
    t6=np.dot(x_trans,temp2);
    #print("x(y-xw)",np.shape(t6))
    t7=-2*t6;
